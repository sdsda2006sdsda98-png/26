  ...
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute left-[8%] top-[12%] h-72 w-72 rounded-full bg-rose-200/35 blur-3xl"
      />
      <motion.div
        animate={{ scale: [1.06, 1, 1.06], opacity: [0.36, 0.54, 0.36] }}
        transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
        className="absolute bottom-[10%] right-[5%] h-80 w-80 rounded-full bg-amber-100/70 blur-3xl"
      />
      {particles.map((particle) => (
        <motion.span
          key={particle.id}
          className="absolute bottom-[-12vh] text-rose-300/45 will-change-transform"
...
function FloatingHeart() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setBeat({
        ...
        scale: 1.06 + Math.random() * 0.1,
        lift: -3 - Math.random() * 7,
  ...
  return (
    <motion.div
      className="pointer-events-none fixed bottom-4 right-4 z-30 grid h-12 w-12 place-items-center rounded-full border border-white/70 bg-white/50 text-rose-400 shadow-[0_18px_54px_rgba(126,91,73,0.16)] backdrop-blur-xl sm:bottom-7 sm:right-6 sm:h-16 sm:w-16"
      animate={{ scale: [1, beat.scale, 1], y: [0, beat.lift, 0], rotate: [0, beat.rotate, 0] }}
      transition={{ duration: beat.duration, repeat: Infinity, ease: "easeInOut" }}
    >
...
function CookieGate({ consent, onAccept, onDecline }) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[90] grid place-items-center px-5 py-8"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      ...
      <motion.div
        className="relative w-full max-w-2xl overflow-hidden rounded-[2rem] border border-white/80 bg-white/76 p-7 text-center shadow-[0_34px_110px_rgba(63,49,44,0.24)] backdrop-blur-2xl sm:p-10"
      ...
      <div className="mx-auto grid w-full max-w-6xl items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          ...
          <button
            ...
            className="mt-10 inline-flex items-center gap-3 rounded-full bg-stone-800 px-8 py-4 text-sm font-semibold uppercase tracking-[0.2em] text-white shadow-[0_22px_60px_rgba(63,49,44,0.28)] transition hover:-translate-y-1 hover:bg-stone-700"
          >
            Открыть
            <Sparkles size={17} />
          </button>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, scale: 1, y: [30, 0, -8, 0] }}
...
function PhotoCard({ memory, index, onOpen }) {
  ...
  return (
    <motion.button
      ...
      className="group relative aspect-[4/5] overflow-hidden rounded-[1.6rem] border border-white/75 bg-white/45 text-left shadow-[0_22px_70px_rgba(126,91,73,0.12)] backdrop-blur-xl"
...
function Lightbox({ photo, onClose }) {
  return (
    <motion.div
      className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/35 p-5 backdrop-blur-xl"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      onClick={onClose}
    >
      <motion.div
        className="relative w-full max-w-3xl overflow-hidden rounded-[2rem] border border-white/65 bg-white/70 p-4 shadow-[0_35px_100px_rgba(55,40,34,0.28)]"
      ...
      >
        ...
        {photo.failed ? (
          ...
          <img src={photo.src} alt={photo.caption} className="max-h-[74vh] w-full rounded-[1.45rem] object-cover" />
        )}
    ...
    <section className="mx-auto max-w-4xl px-5 py-24 text-center sm:px-8">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <motion.div
              className="relative max-w-xl rounded-[2rem] border border-white/70 bg-white/76 p-8 text-center shadow-[0_35px_100px_rgba(55,40,34,0.28)] sm:p-12"
...
function FinalSection() {
  return (
    <motion.footer
      className="px-5 py-32 text-center sm:px-8"
